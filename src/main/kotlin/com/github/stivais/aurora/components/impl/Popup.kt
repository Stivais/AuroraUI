 ...
 * # Popup
 *
 * Extension of [ComponentScope], which contains functions for a popup that can be closed with an animation.
 *
...
class Popup(
    ...
    fun closePopup(smooth: Boolean = this.smooth) {
        if (!closing) {
            ...
            component.ui.addOperation {
                val finished = anim == null || anim.finished
                if (finished) component.ui.main.removeElement(component)
                finished
            }
        }
    }
 ...
 */
inline fun ComponentScope<*>.popup(
